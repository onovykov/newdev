({
    ...
    getStatusReclamation: function(component, event, helper){
       
        var action = component.get("c.getReclamationStatus");
        var recordId = component.get('v.recordId');
        
        action.setParams({'reclamationId':recordId});
        ...
        action.setCallback(this,function(response){
        	...
        	if (response.getState() == "SUCCESS") {
            	...
            	if(status != 'Draft'){
                	component.set('v.buttonConfirmActive', true);
                    
                    let toastEvent = $A.get("e.force:showToast");
                    let toastMsg = $A.get("$Label.c.ReclDetailStageMsg");
                    let toastTitle = $A.get("$Label.c.ReclDetailStageTitle");
    ...
    createReclamationAndReclamationDetails: function (component, event, helper){
 	...
 	if(selectedItems.length != 0){
    	if(validated){
    		...
    		var recordId = component.get('v.recordId');
            var action = component.get("c.addReclamationItems");
        	action.setParams({
            'reclamationId' : recordId,   
         ...
         action.setCallback(this, function(response) {
             if (response.getState() == "SUCCESS") {
                 ...
                 var p = component.get("v.reloadPageParent");
                 p.parentMethod();
                 /** 

    ...
    filterData() {
        let data = this.originalTdata.filter(item => {
            return this.filters.every(filter => {
                ...
                if(filter.type == 'text') {
                    if(String(filter.value).startsWith('>=')) {
                        return (item[filter.api]) >= Number(String(filter.value).replace(/\s/g, '').substring(2));
                    } else if(String(filter.value).startsWith('<=')) {
                        return (item[filter.api]) <= Number(String(filter.value).replace(/\s/g, '').substring(2));
                    } else if(String(filter.value).startsWith('>')) {
                        return (item[filter.api]) > Number(String(filter.value).replace(/\s/g, '').substring(1));
                    } else if(String(filter.value).startsWith('<')) {
                        return (item[filter.api]) < Number(String(filter.value).replace(/\s/g, '').substring(1));
                    } else {
                        return String(String(item[filter.api])).toLowerCase().startsWith(String(filter.value).toLowerCase());
                    }

                } else if (filter.type == 'number' || filter.type == 'picklist' || filter.type == 'tel') {
                    return String(String(item[filter.api])).toLowerCase().startsWith(String(filter.value).toLowerCase());
                } else if (filter.type == 'date' || filter.type == 'datetime' || filter.type == 'color') {

...
import All_enquiries from '@salesforce/label/c.All_enquiries';
import Sent from '@salesforce/label/c.Sent';

import ENQUIRY_OBJECT from '@salesforce/schema/CustomerEnquiry__c';
...
export default class SiteEnquiry extends NavigationMixin(LightningElement) {
    ...
    @wire(getEnquiries,{}) handleEnquiries(result){
        ...
        if(data){
            ...
            let sent = this.filteredEnquiriesList.filter((item=>{
                return item.Status__c == 'Sent';
            }))
            ...
            this.amountSentEnquiries = sent.length;
    ...
    refreshEnquiries() {
        console.log('on child refresh');
        refreshApex(this.enquiriesResult);
    ...
    searchBy(){
        ...
        if(this.isSentEnquiries){
            this.filteredEnquiriesList = this.filteredEnquiriesList.filter(enquiry=>{
                return enquiry.Status__c == 'Sent';
            })
        }
       
    }
    @track options = [
        { id: 'tab-default-3__item', label:  this.labels.All_enquiries, order: 1},
        { id: 'tab-default-6__item', label:  this.labels.Sent, order: 2},
        { id: 'tab-default-1__item', label:  this.labels.Processing   , order: 3},
        { id: 'tab-default-5__item', label:  this.labels.Cancelled   , order: 4 ,},
        { id: 'tab-default-2__item', label:  this.labels.Closed   , order: 5 ,},
        { id: 'tab-default-4__item', label:  this.labels.Draft   , order: 6 ,},
    ];
